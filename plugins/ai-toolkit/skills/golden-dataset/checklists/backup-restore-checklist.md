...
- [Pre-Backup Checklist](#pre-backup-checklist)
- [Backup Execution Checklist](#backup-execution-checklist)
- [Pre-Restore Checklist](#pre-restore-checklist)
- [Restore Execution Checklist](#restore-execution-checklist)
- [Validation Testing Checklist](#validation-testing-checklist)
- [Rollback Checklist (If Restore Fails)](#rollback-checklist-if-restore-fails)
- [Post-Restore Cleanup](#post-restore-cleanup)
...
- [ ] **Database connection verified**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER -c "SELECT version();"
  # Expected: PostgreSQL 16.x
  ```

- [ ] **Database contains expected data**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER -c \
...
- [ ] **URL contract verified (no placeholder URLs)**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER -c \
    "SELECT COUNT(*) FROM analyses WHERE url LIKE '%example.placeholder.com%';"
...
- [ ] **Backup script exists**
  ```bash
  ls -lh $PROJECT_ROOT/backend/scripts/backup_golden_dataset.py
...
- [ ] **Data directory exists**
  ...
  ls -ld $PROJECT_ROOT/backend/data
...
- [ ] **Check backup file created**
  ```bash
  ls -lh $PROJECT_ROOT/backend/data/golden_dataset_backup.json
  # Expected: ~2.5 MB file
  ```

- [ ] **Check metadata file created**
  ```bash
  ls -lh $PROJECT_ROOT/backend/data/golden_dataset_metadata.json
  # Expected: ~1 KB file
  ```
...
- [ ] **Verify backup file is valid JSON**
  ```bash
  cat $PROJECT_ROOT/backend/data/golden_dataset_backup.json | jq '.'
  # Expected: Valid JSON, no parse errors
  ```

- [ ] **Check backup version**
  ```bash
  cat $PROJECT_ROOT/backend/data/golden_dataset_backup.json | \
    jq '.version'
  # Expected: "2.0"
  ```
...
- [ ] **Database accessible**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER -c "SELECT 1;"
  # Expected: "1"
  ```

- [ ] **Current data count known**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER -c "SELECT COUNT(*) FROM analyses;"
...
- [ ] **Sufficient disk space**
  ```bash
  df -h $PROJECT_ROOT/backend/data
...
- [ ] **Check database counts**
  ...
  # Artifacts
  psql -h localhost -p 5437 -U $DB_USER -c "SELECT COUNT(*) FROM artifacts;"
  # Expected: 98

  # Chunks
  psql -h localhost -p 5437 -U $DB_USER -c "SELECT COUNT(*) FROM analysis_chunks;"
  # Expected: 415
  ```
...
- [ ] **Verify fixture files restored** (v2.0 backups only)
  ```bash
  ls -lh $PROJECT_ROOT/backend/tests/smoke/retrieval/fixtures/
  # Expected:
...
- [ ] **Check for SQL dump**
  ```bash
  ls -lh $PROJECT_ROOT/backend/data/golden_dataset_dump.sql
...
- [ ] **Import SQL dump**
  ```bash
  psql -h localhost -p 5437 -U $DB_USER < \
    $PROJECT_ROOT/backend/data/golden_dataset_dump.sql
  ```
