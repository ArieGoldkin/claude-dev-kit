...
### Claude Code Native Cache (CC v2.1.108+)

Claude Code itself supports 1-hour prompt cache TTL via environment variable:

```bash
# Enable 1-hour cache for Claude Code sessions (API key, Bedrock, Vertex, Foundry)
export ENABLE_PROMPT_CACHING_1H=1

# Force 5-minute TTL (override any longer default)
export FORCE_PROMPT_CACHING_5M=1
```

> **Note**: `ENABLE_PROMPT_CACHING_1H_BEDROCK` is deprecated but still honored. Use `ENABLE_PROMPT_CACHING_1H` for all providers.
...
- `llm-patterns` - Streaming with caching
