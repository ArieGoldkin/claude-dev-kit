...
if [ ! -f "$HOOK_RUNNER" ]; then
  echo '{"continue":true,"systemMessage":"⚠ ai-toolkit: compiled hooks not found. Run: cd hooks && npm run build"}'
  exit 0
fi
...
if [ -z "$NODE_BIN" ]; then
  ...
  if [ ! -f "$WARN_MARKER" ]; then
    echo '{"continue":true,"systemMessage":"⚠ ai-toolkit: node not found. Install Node.js or set CLAUDE_NODE_PATH in settings.json env. Checked: PATH, /usr/local/bin, nvm, fnm, volta."}'
