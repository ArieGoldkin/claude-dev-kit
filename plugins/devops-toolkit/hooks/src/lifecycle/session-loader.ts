...
function writeEnvFile(projectDir: string): void {
  ...
  try {
    ...
    if (!process.env['DEVOPS_LOG_LEVEL']) {
      lines.push(`export DEVOPS_LOG_LEVEL=${shellEscape('warn')}`);
    }
...
/**
 * Marker used to identify dtk (formerly devops-toolkit) sections in AGENTS.md.
 */
const DEVOPS_MARKER_START = '<!-- dtk:start -->';

/**
 * Check if AGENTS.md exists with dtk markers.
...
function checkAgentsMd(projectDir: string): string {
  ...
  if (!fs.existsSync(agentsMdPath)) {
    return 'TIP: Run `/generate-agents-md` to index 50+ skills for better AI assistance.\n\n';
  }

  try {
    const content = fs.readFileSync(agentsMdPath, 'utf8');
    if (!content.includes(DEVOPS_MARKER_START)) {
      return 'TIP: Run `/generate-agents-md` to add dtk skills index.\n\n';
    }
  } catch {
