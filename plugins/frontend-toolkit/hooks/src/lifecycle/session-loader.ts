...
/**
 * Marker used to identify ftk (formerly frontend-toolkit) sections in AGENTS.md.
 * Fixes the pre-existing copy-paste bug carried forward from the ai-toolkit→atk
 * rename MR: this plugin now uses its own marker (`<!-- ftk:start -->`) instead
 * of checking for atk's marker.
 */
const FRONTEND_MARKER_START = '<!-- ftk:start -->';

/**
 * Check if AGENTS.md exists with ftk markers.
...
function checkAgentsMd(projectDir: string): string {
  ...
  try {
    ...
    if (!content.includes(FRONTEND_MARKER_START)) {
      return 'TIP: Run `/generate-agents-md` to add ftk skills index.\n\n';
    }
  } catch {
