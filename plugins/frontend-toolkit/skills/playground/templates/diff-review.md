...
- [Layout](#layout)
- [Control types for diff review](#control-types-for-diff-review)
- [Diff rendering](#diff-rendering)
- [Line type styling](#line-type-styling)
- [Comment system](#comment-system)
- [Prompt output format](#prompt-output-format)
- [Data attributes for line elements](#data-attributes-for-line-elements)
- [Pre-populating with real data](#pre-populating-with-real-data)
...
**Grounded by construction**: every diff line, path, and hunk must come from the real `git show`/`git diff` output — never reconstructed from memory. If a fact isn't in the source output, omit it rather than guess; label anything inferred (e.g. a guessed rename) as *inferred*. Give each file's diff section a one-line intent summary above the code so the reviewer reads why before parsing what.
