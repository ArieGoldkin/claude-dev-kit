---
description: Allocate tasks for parallel agent execution. Use when distributing work across multiple parallel agents.
---
...
## Agent Identity
- Agent Type: [ui-developer|devops-architect|ai-ml-engineer]
...
When dispatching subagents, select the appropriate model tier based on task complexity:

| Task Type | Recommended Model | Rationale |
|-----------|------------------|-----------|
| Linting, formatting, simple refactors | `haiku` | Low complexity, fast iteration needed |
| Feature implementation, test writing | `sonnet` | Balanced speed and capability |
| Architecture review, complex debugging | `opus` | Requires deep reasoning and context |
| Code review, spec compliance | `sonnet` | Sufficient for pattern matching |
| Security audit, HIPAA compliance | `opus` | Cannot afford to miss edge cases |

Use the `model` parameter in Agent tool frontmatter to override the default. When in doubt, use `sonnet` — it handles 80% of tasks well.
