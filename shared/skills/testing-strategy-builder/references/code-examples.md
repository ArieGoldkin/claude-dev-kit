...
- [Unit Test Examples](#unit-test-examples)
  - [Basic Function Testing](#basic-function-testing)
...
- [End-to-End Test Examples](#end-to-end-test-examples)
  ...
  - [Test Selector Best Practices](#test-selector-best-practices)
- [Performance Test Examples](#performance-test-examples)
  - [Load Test with k6](#load-test-with-k6)
- [Test Data Management Examples](#test-data-management-examples)
  - [Test Factory](#test-factory)
  - [Test Fixtures](#test-fixtures)
- [Mocking Examples](#mocking-examples)
  - [Mocking External API](#mocking-external-api)
- [Snapshot Testing](#snapshot-testing)
- [Parameterized Tests](#parameterized-tests)
- [Test Isolation Example](#test-isolation-example)
...
**IMPORTANT:** Always use semantic `data-testid` attributes for E2E testing. See [test-selectors.md](../../instructions/test-selectors.md) for complete guide.

**Pattern: Interactive elements with test IDs**

```tsx
export const InfoPageView = ({ title, benefits, onContinue }) => {
  return (
    <div data-testid="info-page-view">
      <h2 data-testid="info-page-view-title">{title}</h2>

      <BenefitsList benefits={benefits} />

      <button
        onClick={onContinue}
        data-testid="info-page-view-cta-button"
      >
        Continue
      </button>
    </div>
...
**Pattern: List items with unique IDs**

```tsx
import { createTestId } from '@utils';

const BenefitCard = ({ benefit }: { benefit: string }) => {
  const testId = createTestId(benefit); // "improve-your-focus"

  return (
    <li data-testid={`info-page-view-benefit-${testId}`}>
      {benefit}
    </li>
  );
};
...
export const BenefitsList = ({ benefits }: { benefits: string[] }) => {
  return (
    <ul data-testid="info-page-view-benefits-list">
...
**Pattern: Using test IDs in Playwright tests**

```typescript
test('verify info page content', async ({ page }) => {
  await page.goto('/action/mindfulness/info');

  // Test interactive element
  await page.click('[data-testid="info-page-view-cta-button"]');

  // Test list container
  await expect(
    page.locator('[data-testid="info-page-view-benefits-list"]')
  ).toBeVisible();

  // Test specific list item with unique ID
  await expect(
    page.locator('[data-testid="info-page-view-benefit-improve-your-focus"]')
  ).toBeVisible();
});
```

**Why use `createTestId` for list items?**
- ✅ Stable: Same content = same ID every time
- ✅ Semantic: QA knows which item they're testing
- ✅ Survives reordering: Not index-based
- ❌ Never use index: `benefit-${index}` breaks on reorder

**Complete patterns:** See [test-selectors.md](../../instructions/test-selectors.md)
