...
description: |
  Full-stack performance analysis, optimization patterns, and monitoring strategies for
  platform. Use when optimizing slow database queries, reducing frontend bundle
  size, improving Core Web Vitals (LCP, INP, CLS), fixing N+1 query problems, or preparing
  for high traffic. Covers PostgreSQL query optimization with EXPLAIN ANALYZE, React
  performance profiling, code splitting and lazy loading, caching strategies (Redis, CDN),
  and load testing with k6. Handles API latency reduction, memory leak detection, and
...
> See `${CLAUDE_SKILL_DIR}/templates/database-optimization.ts` for N+1 fixes and pagination patterns
...
> See `${CLAUDE_SKILL_DIR}/templates/caching-patterns.ts` for full implementation
...
> See `${CLAUDE_SKILL_DIR}/templates/frontend-optimization.tsx` for patterns
...
> See `${CLAUDE_SKILL_DIR}/templates/api-optimization.ts` for middleware examples
...
> See `${CLAUDE_SKILL_DIR}/templates/performance-metrics.ts` for Prometheus metrics setup
